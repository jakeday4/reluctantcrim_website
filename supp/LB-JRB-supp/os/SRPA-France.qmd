...
est2dat %>%
  dplyr::filter(!CMA_est %in% c("pm", "pe", "ERpnie(prop)", "ERintref(prop)",
...
# datv3trim %>% dplyr::select(
#   anyvic, SRPAavgz, eversex, SRintelz, SRpopularz, agez, worked, gender
#   ) %>%
#   datasummary_skim()

datv3trim <- datv3 %>% 
  dplyr::select(
    sexvicsum, SRPAavgz, eversex, SRintelz, SRpopularz, agez, worked, gender
    ) %>%
  mutate(anyvic = if_else(sexvicsum > 0, 1, sexvicsum), 
