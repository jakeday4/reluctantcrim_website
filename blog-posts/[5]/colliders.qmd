...
For instance, using the fourth example above, we can identify all paths between *X* and *Y* and [identify appropriate adjustment sets](https://evalf20.classes.andrewheiss.com/example/dags/#paths-and-adjustment-sets) automatically using `ggdag::ggdag_paths()` and `ggdag::ggdag_adjustment_set()`. You will note below that the path from *X to Y* and *X to C to Y* are open or unblocked paths, but the path from *X to Z to C to Y* is not an open path - unless we open it by conditioning on *Z* and, in turn, bias our estimate of the effect of *X* on *Y*. Likewise, the appropriate adjustment set for Example 4 contains only *C*, meaning we should control for *C* but not *Z*.   
...
While SAT actually implies a complex causal structure involving a three-way (or more) interaction between moral beliefs, moral context, and self-control, we will specify a simpler DAG for the purposes of illustrating collider biases. Instead, we will aim to specify a causal structure and simulate data that builds upon the logic of many tests of SAT, which often regress delinquency or crime on self-control, moral beliefs, and their multiplicative interaction, as well as include gender and "controls" for other (presumably independent) variables. Hence, a typical SAT test might implicitly assume a causal structure reflected in the following directed acyclic graph, or DAG: 

```{r}
#| label: DAG4
#| fig-cap: "Figure 5. Implied causal diagram (DAG) underlying a typical SAT test"

SATDAG <- dagitty('dag{
  Male -> {SelfControl MoralBeliefs Delinquency}
  SelfControl -> {Delinquency}
  MoralBeliefs -> {Delinquency} 
  Controls -> {Delinquency} 
   }') 
coordinates( SATDAG ) <-
    list( x=c(SelfControl=1, Male=1, MoralBeliefs=1, Controls=1, Delinquency=2),
        y=c(SelfControl=1, Male=0, MoralBeliefs=-1, Controls=-2, Delinquency=0) )
...
![Figure 6: Self-control and morality interact to predict delinquency](Interactions.jpg)

Yet, as we think is true in tests of most criminological theories, empirical tests of SAT often explicitly consider only a limited set of causal relationships within a data source generated by a much larger connection of complex interlinked causal processes - much more than those depicted in the Figure 5 DAG above. So, we will add a bit of plausible complexity to the DAG to illustrate how colliders might bias estimation of theorized causal effects.  
...
```{r}
#| label: DAG5
#| fig-cap: "Figure 7. Causal diagram (DAG) used to generate simulated data"
...
Success! The plot on the left confirms that our simulation recreated the moderated effects of self-control on delinquency across levels of moral beliefs as documented in earlier research (e.g., see Figure 6 above, though note our x-axis is reversed and we specified a nonlinear functional form). 
...
Note, too, that there is an estimated negative effect of employment on delinquency despite a lack of simulated causal relationship between them. This is because both employment and delinquency "listen to" (in this case, are caused by) common mechanisms, namely moral beliefs and self-control (see [Pearl & Mackenzie](http://bayes.cs.ucla.edu/WHY/) for explanation & more on the "listens to" metaphor). 

Of course, if we had not simulated the data ourselves or had a reliable DAG to communicate our causal assumptions - which is exactly the situation most researchers find themselves in - then we might simply assume that these causal estimates are accurate and then proceed to tell a story about them in a paper for publication. 
...
What would happen if we continue to condition on the collider QualityEmploy in our other simulated sample that also essentially conditions on felony status, or a key (simulated) cause of employment quality/status? Let's reestimate our two models using the simulated felony sample to see.
...
Additionally, employment is incorrectly predicted to have a sizeable negative effect on delinquency in both the general and felony samples, despite a lack of causal connection or direct simulated correlation between them. A review of our DAG helps us understand why this is the case: we did not adjust for self-control and moral beliefs, two confounders of the employment/delinquency relationship. 

Finally, a large, positive estimate of the effect of being male on delinquency is observed in the general sample, whereas a smaller positive effect estimate is observed in the felony sample. This is likely because, due to our DAG and simulation, being male is causally related to self-control and moral beliefs. Thus, in the general sample, being male is expected to have a positive total effect due to its unblocked *indirect* effects on delinquency through these mediators. In contrast, in the felony sample, these indirect effect estimates are affected by conditioning on a collider (felony).
...
[^6]: "Avoid being clever" is another McElreath motto. You're probbably picking up on a pattern with all of our McElreath references. Seriously, do yourself a favor and read his book and watch his lectures. 

Specifically, for any theoretical estimand you want to empirically estimate from the data, you can start by designing your DAG with a program like "DAGitty" and identifying the exposure (cause) and outcome. Then, you can have the program automatically identify the various adjustment sets that would give you the unbiased estimate based on your theoretical model as specified in the DAG. The [DAGitty User's Manual]() explains that: 

> "...any set of variables in a causal diagram that blocks all biasing paths between an exposition and an outcome, but leaves the causal paths open, is called a *sufficient adjustment set*. **If the causal diagram is accurate**, then adjustment, stratification, or selection (e.g. by restriction or matching) for this set of variables in an epidemiological study minimizes bias when estimating the *total effect* from exposition to outcome..." (p.2, bold emphasis added.)
...
```{r}
#| label: DAG6
#| fig-cap: "Figure 8. Causal diagram (DAG), highlighting Example 3 estimand"
