...
One of these sets does includes adjusting only for male without employment in the model. In fact, you might be surprised to see that, in the case of the SES -> Delinquency relationship, the minimal and arguably most sensible of the valid adjustment sets includes adjusting for nothing at all ("{}")! There is an important lesson here: Depending on the DAG, it might be quite literally better to include no control variables at all in a model than it is to include the wrong controls. 

![*Try not* to control for stuff. Use the force DAG. *Do* control. *Or do not* control. *There is no try*.](notry.png)
