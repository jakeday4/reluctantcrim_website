...
Again, I will focus here on the last two issues (#2 and #3).[^2] For those interested in reading my detailed narrative review describing these issues, click below to expand. Otherwise, I will briefly summarize the issues again so feel free to move on. 
...
[This issue](https://x.com/PWGTennant/status/1806032426440823012) is [an old one](https://x.com/PWGTennant/status/1720108111132844091) that applies to [ratio measures in general](https://www.jstor.org/stable/2983064?origin=crossref), as predictors or [as outcomes](https://elevanth.org/blog/2022/09/02/there-are-no-magic-outcome-variables/), and it is particularly important to consider when [investigating causal questions](https://academic.oup.com/aje/advance-article/doi/10.1093/aje/kwae162/7699740?login=false) (which most quantitative criminology studies do [whether or not they admit it](https://x.com/PWGTennant/status/1681938373206835200)), irrespective of one's [theoretical interest](https://www.jstor.org/stable/2095384?origin=crossref) in the ratio. If you think a ratio variable measures something distinct from the constituent variables comprising the ratio, then you should be sure your design identifies the causal effect of that distinct "thing" in a way that isolates it (if possible) from any entangled causal effects of the constituent variables comprising the ratio measure. 
...
Marginal effects plots show a very strong and nonlinear effect of offending on perceived rewards. Recall, I specified the effects of offending and arrest as linear on the estimation scale (quadratic *B* terms = 0). Well, notice that the estimated effect of offending does not depart substantially from linearity on the estimation scale, as evidenced by a near-zero quadratic term (*b*= -0.003); the same is true for the estimated effect of arrest (quadratic term *b*= -0.005). Meanwhile, the linear parameter estimates nearly recover the parameters used to simulate the data. As mentioned earlier and [discussed elsewhere](https://reluctantcriminologists.com/blog-posts/%5B6%5D/modmadness-pt1), nonlinearities in predicted outcome and marginal effects plots might reflect transformations on *outcome* (not estimation) scales when plotting results from models with nonlinear (log) link functions and non-Gaussian (Poisson) distributed variables. In this case, the outcome data were generated partly as a function of a positive linear relationship between Poisson-distributed offending scores, which baked in a strong nonlinear association on the outcome scale. Additionally, censoring decisions added nonlinear artifacts and also affected model fit. The quadratic terms added some flexibility to the model, allowing it to fit the nonlinear patterns in the data better than a linear model.
